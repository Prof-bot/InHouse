...
private _presetName = format ["wmi_preset%1", _network];
...

{
    if (player canAdd _radio) then {
        ...
        systemChat format ["[WMI] Added radio: %1", _radio];
    } else {
        if (getContainerMaxLoad uniform player > 0) then {
            (uniformContainer player) addItemCargoGlobal [_radio, 1];
            systemChat format ["[WMI] Added radio (exceeds inventory capacity): %1", _radio];
        } else {
            systemChat format ["[WMI] Couldn't add radio: %1", _radio];
        };
