...
wmi_localRespawnedUnit = objNull;
/// Create the unit
private _unitName = format["respawnedUnit%1",_number];

private _init = format["if (local this) then { [this, '%2', '%3'] call wmi_assignGear_fnc_assignGear; wmi_localRespawnedUnit = this; }; this setName '%4'; this setVariable ['wmi_isRespawnUnit',true];  %1 = this; ",_unitName, _faction, _typeOfUnit, profileName];
private _oldUnit = player;

_class createUnit [_position, _dummyGroup, _init, 0.5, _rankName];    

// Wait till the unit is created
waitUntil{!isNull wmi_localRespawnedUnit};
...
setPlayable wmi_localRespawnedUnit; // Allow unit to referenced later with player functions (name/getPlayerUID) (Verify this works)
selectPlayer wmi_localRespawnedUnit;
...
if (isClass(configFile >> "CfgPatches" >> "acre_main")) then  {
    ...
    [{
        ...
        if (isNil "wmi_acre2_networksCreated") exitWith {}; //Ensure presets are created
