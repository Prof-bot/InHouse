...
/*
 * Name: WMI_spectator_fnc_drawGroupMarker
 * Author: Prof
 *
 * Arguments:
...
params ['_map', '_grp','_color', '_pos'];
 private _wmiGrpMarkerData = [_x] call EFUNC(orbat,getGroupMarkerData);
// if we have a framework orbat marker, use it.
if(count _wmiGrpMarkerData >= 3) then {
    _wmiGrpMarkerData params ["_grpTexture","_gname","_grpTextureSize"];
