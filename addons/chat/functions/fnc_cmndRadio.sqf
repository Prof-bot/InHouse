...
if (_name isEqualTo "") exitWith {systemChat "WMI Error: No argument passed. Command usage: #radio <radioType>"};
...
if (_radios isEqualTo []) exitWith {
    systemChat FORMAT_1("WMI Error: Unable to find radio containing %1", str _name);
};
if (count _radios > 1) exitWith {
    systemChat FORMAT_1("WMI Error: More than one radio found containing %1", str _name);
};
...
if (CURUNIT canAdd _radioType) then {
    ...
    systemChat FORMAT_1("WMI: Added %1 to inventory", _radioName);
} else {
    systemChat FORMAT_1("WMI Error: Not enough room for %1 in inventory", _radioName);
};
