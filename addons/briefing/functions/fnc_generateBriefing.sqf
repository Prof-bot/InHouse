...
private _briefingArray = getMissionConfigValue ["WMI_Briefing","[]"];
...
private _groupCond = _unitGroup getVariable ["WMI_Briefinglist", []];
if (_groupCond isEqualType "") then { _groupCond = call compile _groupCond; };
private _unitCond = _unit getVariable ["WMI_Briefinglist", []];
...
//Process scripts.
{
    ...
    if FILE_EXISTS(_scriptName) then {
        ...
        [_scriptName] spawn {
            params ["_scriptName"];
            uiSleep 5;
            systemChat format["[WMI-Briefing] Missing file: %1 ", _scriptName];
        };
    };
...
if ([] call wmi_common_fnc_isAdmin) then {
    if FILE_EXISTS("briefing\admin.sqf") then {
        ...
        [] spawn {
            uiSleep 5;
            systemChat "[WMI-Briefing] Warning admin briefing not found. Expected: MISSION_ROOT\briefing\admin.sqf";
        };
    };
};
