...
fn_removeUnitFromBrief = {
    ...
    _list = (_unit get3DENAttribute "WMI_Briefinglist") params ["_value"];
    if (_value isEqualType []) then {
        //Is default do nothing
        _//unit set3DENAttribute ["WMI_Channellist",str []];
    } else {
        _value = call compile _value;
        _value = _value - [_channel];
        _unit set3DENAttribute ["WMI_Briefinglist",str _value];
    };
};
...
fn_removeGroupFromBrief = {
    ...
    _list = (_group get3DENAttribute "WMI_Briefinglist") params ["_value"];
    if (_value isEqualType []) then {
        //Is Default do nothing
        //_group set3DENAttribute ["WMI_ChannellistLeader",str []];
    } else {
        _value = call compile _value;
        _value = _value - [_curSel];
        _group set3DENAttribute ["WMI_Briefinglist",str _value];
    };
    ...
    if (_value isEqualType []) then {
        //is default do nothing
        //_group set3DENAttribute ["WMI_Channellist",str []];
    } else {
        _value = call compile _value;
        _value = _value - [_curSel];
        _group set3DENAttribute ["WMI_Briefinglist",str _value];
...
switch _mode do {
    case "onLoad": {
        ...
        BriefingArray = ("WMI_MissionBriefingAttributes" get3DENMissionAttribute "WMI_Briefing");
            ...
            //Deserialize number to side.
            {
                _x params ["","_conditions"];
                {
                    if (_x isEqualType 0) then {
                        _conditions set [_forEachIndex, (_x call WMI_common_fnc_numToSide)];
                    };
                } forEach (_conditions);
        ...
        private _array = + (uiNamespace getVariable "BriefingArray");
        {
            _x params ["","_conditions"];
            {
                if (_x isEqualType east) then {
                    _conditions set [_forEachIndex, (_x call WMI_common_fnc_sideToNum)];
                };
            } forEach (_conditions);
        } forEach _array;
    ...
    case "refreshBriefTree": {
        ...
        fn_BriefTreeProcessUnit = {
            ...
            private _color = (side _unit) call WMI_common_fnc_sideToColor;
            ...
            if (!_doSpeak) then {
                private _unitChanList = (_unit get3DENAttribute "WMI_Briefinglist") select 0;
            ...
            if (_doSpeak) then {
                _ctrlTree tvSetPictureRight [_location, "x\wmi\addons\briefing\UI\check_small_ca.paa"];
                _returnCode = 0;
            } else {
                _ctrlTree tvSetPictureRight [_location, "x\wmi\addons\briefing\UI\plus_small_ca.paa"];
            };
        ...
        fn_BriefTreeProcessGroup = {
            ...
            if (_render) then {
                private _color = _side call WMI_common_fnc_sideToColor;
            ...
            if (!_doSpeak) then {
                private _grpChanList = (_group get3DENAttribute "WMI_Briefinglist") select 0;
        ...
        private _sides = []; {_sides pushBackUnique (side _x);} forEach cacheAllPlayerGroups;
        {
            ...
            private _location = [(_ctrlTree tvAdd [[], _side call WMI_common_fnc_sideToString])];
            
            _ctrlTree tvSetPicture [_location, _side call WMI_common_fnc_sideToTexture];
        ...
        with uiNamespace do {
            ...
            if (_entity isEqualType east or _entity isEqualType "") then {
                ...
                if (_entity isEqualType grpNull or _entity isEqualType objNull) then {
                    _list = (_entity get3DENAttribute "WMI_Briefinglist") params ["_value"];
                    if (_value isEqualType []) then {
                        _entity set3DENAttribute ["WMI_Briefinglist",str [_curSel]];
                    } else {
                        _value = call compile _value;
                        _value pushBackUnique _curSel;
                        _entity set3DENAttribute ["WMI_Briefinglist",str _value];
                    };
                };
            };
        ...
        with uiNamespace do {
            ...
            if (_entity isEqualType east or _entity isEqualType "") then {
                ...
                if (_entity isEqualType east) then {
                    ...
                    private _sideNum = _entity call WMI_common_fnc_sideToNum;
                    {
    ...
    case "save": {
        ...
        private _string = str _array;
        set3DENMissionAttributes [["WMI_MissionBriefingAttributes","WMI_Briefing",_string]];
    };
};
