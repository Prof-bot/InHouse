...
uiNamespace setVariable ["wmi_assignGear_validFaces",_faceClasses];

{
    ...
    if (count _weightedArray > 0) then {
        uiNamespace setVariable ["wmi_assignGear_faceset_"+_name,_weightedArray];
    };
} forEach ("true" configClasses (configFile >> "wmi_assignGear_facesets"));
