...
private _vehicles = [];
{
    ...
    if (_condition isEqualType 0 and {(_condition call EFUNC(common,numToSide)) == (side _unit)}) exitWith {
        ...
        _vehicles = vehicles select {((_x getVariable ["wmi_orbat_team",""]) param [0,""]) isEqualTo _sideStr};
    };
    ...
    if ((faction (leader (group _unit)) isEqualTo _condition)) exitWith {
        ...
        _vehicles = vehicles select {((_x getVariable ["wmi_orbat_team",""]) param [0,""]) isEqualTo (toLower _faction)};
    };
...
{
    private _var = _x getVariable ["WMI_OrbatParent",-1];
...
_fnc_processOrbatTrackerBriefingRawData = {
    ...
    // Process Children (Groups)
    {
        ...
        if (_id == _uniqueID) then {
            private _markerEntry = _entity getVariable ["WMI_groupMarker",[]];
            ...
            if (_groupTexture == "") then {
                if (_texture1 != "") then {
                    ...
                    _groupTexture = "\x\wmi\addons\orbat\textures\empty.paa";
                };
            };
            ...
            if (_entity isEqualType objNull) then {
                ...
                if (_groupTexture == "\x\wmi\addons\orbat\textures\empty.paa") then {
                ...
                _thisBriefing = _thisBriefing + "<br/>";
                {
                    ...
                    private _markerEntry = _x getVariable ["WMI_SpecialistMarker",[]];
                    if (_markerEntry isEqualType "") then { _markerEntry = call compile _markerEntry; };
                    ...
                    if (primaryWeapon _x != "") then {
                        ...
                        private _weaponIcon = switch ((([primaryWeapon _x] call BIS_fnc_itemType) select 1)) do {
                            case "MachineGun": {"\x\wmi\addons\orbat\weapon_textures\autorifle"};
                            case "BombLauncher";
                            case "Cannon";
                            case "Launcher";
                            case "RocketLauncher": {
                                if ((getNumber (configFile >> "CfgWeapons" >> (primaryWeapon _x) >> "rhsdisposable") == 1) or
                                    ...
                                    (isText (configFile >> "CfgWeapons" >> (primaryWeapon _x) >> "ACE_UsedTube"))) then {"\x\wmi\addons\orbat\weapon_textures\lat"} else {"\x\wmi\addons\orbat\weapon_textures\mat"}
                                };
                            case "GrenadeLauncher": {"\x\wmi\addons\orbat\weapon_textures\gl"};
                            case "MissileLauncher": {"\x\wmi\addons\orbat\weapon_textures\hat"};
                            case "SniperRifle":{"\x\wmi\addons\orbat\weapon_textures\sniper"};
                            case "SubmachineGun":{"\x\wmi\addons\orbat\weapon_textures\smg"};
                            case "Shotgun":{"\x\wmi\addons\orbat\weapon_textures\shotgun"};
                            case "AssaultRifle"; //{"\x\wmi\addons\orbat\weapon_textures\carbine"}; carbine too hard to know
                            default {if (_bipod != "") then {"\x\wmi\addons\orbat\weapon_textures\dmr"} else { "\x\wmi\addons\orbat\weapon_textures\rifleman"}};
                            //"AssaultRifle"
                        };
                        // Handle grenadiers
                        if (count (getArray (configFile >> "CfgWeapons" >> (primaryWeapon _x) >> "muzzles") - ["SAFE"]) > 1) then {
                            _weaponIcon = "\x\wmi\addons\orbat\weapon_textures\rifleman_ugl";
                        };
                        ...
                        private _weaponIcon = switch ((([secondaryWeapon _x] call BIS_fnc_itemType) select 1)) do {
                            ...
                            case "RocketLauncher": {
                                if ((getNumber (configFile >> "CfgWeapons" >> (secondaryWeapon _x) >> "rhs_disposable") == 1) or
                                    ...
                                    (isText (configFile >> "CfgWeapons" >> (secondaryWeapon _x) >> "ACE_UsedTube"))) then {"\x\wmi\addons\orbat\weapon_textures\lat"} else {"\x\wmi\addons\orbat\weapon_textures\mat"}
                                };
                            case "GrenadeLauncher": {"\x\wmi\addons\orbat\weapon_textures\gl"};
                            case "MissileLauncher": {"\x\wmi\addons\orbat\weapon_textures\hat"};
                            case "SniperRifle":{"\x\wmi\addons\orbat\weapon_textures\sniper"};
                            case "AssaultRifle"; //{"\x\wmi\addons\orbat\weapon_textures\carbine"}; Need a method to find if a weapon is a carbine
                            case "SubmachineGun":{"\x\wmi\addons\orbat\weapon_textures\smg"};
                            case "Shotgun":{"\x\wmi\addons\orbat\weapon_textures\shotgun"};
                            default {};
                        };
                    ...
                    if (handgunWeapon _x != "") then {
                        ...
                        private _weaponIcon = "\x\wmi\addons\orbat\weapon_textures\pistol_st.paa";
                        if (_muzzel != "") then {
                            _weaponIcon = "\x\wmi\addons\orbat\weapon_textures\pistol_su.paa";
                            if (_optic != "") then {
                                _weaponIcon = "\x\wmi\addons\orbat\weapon_textures\pistol_al.paa";
                            };
                        } else {
                            if (_optic != "") then {
                                _weaponIcon = "\x\wmi\addons\orbat\weapon_textures\pistol_sc.paa";
                            };
                        };
