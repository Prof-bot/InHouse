...
private _sizeMod = ["x\wmi\addons\orbat\textures\empty.paa", "x\wmi\addons\orbat\textures\modif_o.paa","x\wmi\addons\orbat\textures\modif_dot.paa","x\wmi\addons\orbat\textures\modif_2dot.paa","x\wmi\addons\orbat\textures\modif_3dot.paa", "x\wmi\addons\orbat\textures\modif_company.paa"];

private _icon = _toolBoxIcon select (lbcursel _ctrlIconToolbox);
private _path = "";

if (_icon != "") then {
    _path = "x\wmi\addons\orbat\textures\" + (_colours select  (lbcursel _ctrlColourToolBox)) + _icon;
};
...
_entity set3DENAttribute ["WMI_groupMarker",str _groupMarkerArray];
