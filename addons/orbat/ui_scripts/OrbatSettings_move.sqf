...
with uiNamespace do {
    ...
    if (OrbatSelection isEqualType east) then {
        ...
        _relevantVehicles = vehicles select {((_x get3DENAttribute "wmi_orbat_team") param [0,""]) isEqualTo _sideStr};
    };
    if (OrbatSelection isEqualType "") then {
        _relevantGroups = (cacheAllPlayerGroups select {faction (leader _x) == OrbatSelection});
        _relevantVehicles = vehicles select {((_x get3DENAttribute "wmi_orbat_team") param [0,""]) isEqualTo (toLower OrbatSelection)};
    };
    ...
    _playableUnits = _playableUnits arrayIntersect _playableUnits;
    {
        if (_x in cacheAllPlayerGroups) then {
            private _var = (_x get3DENAttribute "WMI_OrbatParent") # 0;
            if (_var != -1) then {
            ...
            {
                private _markerEntry = (_x get3DENAttribute "WMI_SpecialistMarker") # 0;
    ...
    wmi_fnc_findUsedIDs = {
        ...
        {
            _x call wmi_fnc_findUsedIDs;
        } forEach _children;
    };
    _rootEntry call wmi_fnc_findUsedIDs;
    wmi_fnc_findUsedIDs = nil;
    {
        private _var = (_x get3DENAttribute "WMI_OrbatParent") # 0;
        if (_var != -1) then {
            if (_var in _orbatTreeUsedIDs) then {
                ...
                _x set3DENAttribute ["WMI_OrbatParent", -1];
                _toPlace pushBack [_reserveId, _x];
            };
        } else {
            _toPlace pushBack [_reserveId, _x];
...

        {
            ...
            if (_id == _uniqueID) then {
                if (_entity isEqualType grpNull || {_entity in vehicles}) then {
                    private _markerEntry = (_entity get3DENAttribute "WMI_groupMarker") # 0;
                    if (_markerEntry isEqualType "") then { _markerEntry = call compile _markerEntry; };
                    private _sortId = 0;
                    ...
                    if (_entity isEqualType objNull) then {
                        private _markerEntry = (_entity get3DENAttribute "WMI_SpecialistMarker") # 0;
                        if (_markerEntry isEqualType "") then { _markerEntry = call compile _markerEntry; };
                        private _sortId = 0;
        ...
        private _found = false;
        {
            ...
            if (_entry isEqualTo _value) exitWith {
                ...
                if (_entry isEqualType grpNull || {_entry in vehicles}) then {
                    ...
                    private _markerEntry = (_entry get3DENAttribute "WMI_groupMarker") # 0;
                    if (_markerEntry isEqualType "") then { _markerEntry = call compile _markerEntry; };
                    ...
                    _entry set3DENAttribute ["WMI_groupMarker",str _markerEntry];
                } else {
                    if (_entry isEqualType objNull) then {
                        private _markerEntry = _entry getVariable ["WMI_SpecialistMarker",[]];
                        if (_markerEntry isEqualType "") then { _markerEntry = call compile _markerEntry; };
                        if (count _markerEntry == 0) then {
                            _markerEntry = ["","",0];
                        };
                        ...
                        _entry set3DENAttribute ["WMI_SpecialistMarker",str _markerEntry]
                    };
                };
                // Swap
                ...
                if (_swapEntry isEqualType grpNull || {_swapEntry in vehicles}) then {
                    private _markerEntry = (_swapEntry get3DENAttribute "WMI_groupMarker") # 0;
                    if (_markerEntry isEqualType "") then { _markerEntry = call compile _markerEntry; };
                    ...
                    _swapEntry set3DENAttribute ["WMI_groupMarker",str _markerEntry];
                } else {
                    if (_swapEntry isEqualType objNull) then {
                        private _markerEntry = (_swapEntry get3DENAttribute "WMI_SpecialistMarker") # 0;
                        if (_markerEntry isEqualType "") then { _markerEntry = call compile _markerEntry; };
                        if (count _markerEntry == 0) then {
                            _markerEntry = ["","",0];
                        };
                        ...
                        _swapEntry set3DENAttribute ["WMI_SpecialistMarker",str _markerEntry]
                    };
                };
            };
        } forEach orbat_queue;
