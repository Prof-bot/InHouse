    ...
    fn_processTreeEntry = {
            ...
            if (_id == _uniqueID) then {
                if (_entity isEqualType grpNull || {_entity in vehicles}) then {
                    ...
                    if (_returnedId != _sortId) then {
                        ...
                        _entity set3DENAttribute ["WMI_groupMarker",str _markerEntry]
                    };
                        ...
                        if (count _markerEntry > 0) then {
                            ...
                            if (_mTexture != "") then {
                                ...
                                if (_returnedId != _sortId) then {
                                    ...
                                    _entity set3DENAttribute ["WMI_SpecialistMarker",str _markerEntry]
                                };
                            };
                        };/* else {
                            _markerEntry = ["","",0];
...

        {
            ...
            if (_entry isEqualType grpNull || _isVeh) then {
                private _groupMarkerData =  (_entry get3DENAttribute "WMI_groupMarker") select 0;
                if (_groupMarkerData isEqualType "") then { _groupMarkerData = call compile _groupMarkerData; };

                private _string = "";
                if (_isVeh) then {
                    // Get callsign.
                    private _callsign = (_entry get3DENAttribute "WMI_orbat_vehicleCallsign") param [0,""];
                ...
                if (_entry isEqualType objNull) then {
                    private _unitMarkerData =  (_entry get3DENAttribute "WMI_SpecialistMarker") select 0;
