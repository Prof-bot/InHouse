...
class GVAR(garrison) : Module_F
{
    ...
    category = "WMI";
    icon = "\x\wmi\addons\common\UI\logo_wmi_small_ca.paa";
...
class GVAR(garrisonQuantity) : Module_F
{
    ...
    canSetArea=1;
    class EventHandlers {
        init = "if(isServer && !is3DEN) then {[{_this call wmi_AI_fnc_garrisonQuantity;}, [_this select 0,[],false]] call CBA_fnc_execNextFrame;};_this call bis_fnc_moduleInit;";
    };
